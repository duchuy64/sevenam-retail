(() => {
  ...
  const EXPORT_2K = { w:2046, h:1364, tag:'2K' };
  const EXPORT_4K = { w:4092, h:2728, tag:'4K' };
  ...
  const ICON_ASSETS = {
    ...
    completion_green:'assets/icons/completion.svg',
    average_day:'assets/icons/average-day.svg',
    forecast:'assets/icons/forecast.svg',
    trophy:'assets/icons/trophy.svg',
    comment:'assets/icons/comment.svg',
    schedule:'assets/icons/schedule.svg',
    gap:'assets/icons/gap.svg',
  ...
  function preloadIconAssets(){
    const jobs = Object.entries(ICON_ASSETS).map(
      ([key,src]) =>
        new Promise(resolve => {
          const img = new Image();
          ...
          img.onerror = () => {
            console.warn('Không tải được icon',src);
            resolve();
          };
...
\tTổng\t7.010\t2.839,1\t2.713,5\t125,6\t219,5\t41%`;
  ...
  function normHeader(s){
    return stripAccents(cleanCell(s))
      .replace(/[\n\r]+/g,' ')
      .replace(/\s+/g,' ')
      .trim();
  }
  ...
  function statusDisplay(diff){
    if(diff < 0){
      return `Chậm ${fmtStatusDelta(diff)}`;
    }

    if(diff <= 10){
      return `Kịp ${fmtStatusDelta(diff)}`;
    }

    return `Vượt ${fmtStatusDelta(diff)}`;
  }
  ...
  function findHeader(lines){
    for(
      ...
      if(
        n.includes('thuc dat') &&
        n.includes('hoan thanh tg thang') &&
        (
          n.includes('tg t') ||
          n.includes('target')
        )
    ...
    throw new Error(
      'Không tìm thấy dòng tiêu đề có TG T8/TG tháng, Thực đạt và Hoàn thành TG tháng.'
    );
  }
  ...
  function columnMap(headers){
    ...
    headers.forEach(
      (h,i) => {
        ...
        if(
          ...
          n.includes('ds can dat')
        ){
          map.need = i;
        }

        else if(
          n.includes('thua thieu')
        ){
          map.surplus = i;
        }

        else if(
          n.includes('target can chay') ||
          n.includes('can chay 1 ngay')
        ){
          map.perDay = i;
        }
    ...
    if(
      ...
      throw new Error(
        'Thiếu cột bắt buộc: SR, TG T8/TG tháng, Thực đạt hoặc Hoàn thành TG tháng.'
      );
    }
  ...
  function parsePastedData(text){
    ...
    if(!rows.length){
      throw new Error(
        'Không đọc được dữ liệu cửa hàng.'
      );
    }
    ...
    if(!updateDate){
      ...
      dateSource =
        'Ngày hệ thống – tự động';
    }


    const autoClosingDate =
      new Date(updateDate);


    autoClosingDate.setDate(
      autoClosingDate.getDate()-1
    );
    ...
    if(totalRow){
      if(
        ...
        issues.push({
          type:'warn',
          text:
            `Tổng TG tháng dán ${fmt1(totalRow.target)} lệch tổng 16 cửa hàng ${fmt1(sumTarget)}.`
        });
      }
      ...
      if(
        ...
        issues.push({
          type:'warn',
          text:
            `Tổng Thực đạt dán ${fmt1(totalRow.actual)} lệch tổng 16 cửa hàng ${fmt1(sumActual)}.`
        });
      }
  ...
  function statusFromDiff(diff){
    if(diff < -10){
      return {
        ...
        color:C.red,
        note:
          'Thấp hơn kế hoạch trên 10 điểm %'
      };
    }
  ...
  function renderValidation(){
    ...
    lines.push(
      `<div class="ok">✓ Nhận ${m.rows.length} cửa hàng • Target T${m.targetMonth}: ${fmt1(m.systemTarget)}</div>`
    );


    lines.push(
      `<div class="ok">✓ Ngày cập nhật: ${dateVN(m.updateDate)} • chốt ${dateVN(m.closingDate)} • ${m.currentDay}/${m.totalDays} ngày</div>`
    );


    lines.push(
      `<div class="muted">Nguồn ngày: ${escapeHtml(m.dateSource)}</div>`
    );


    lines.push(
      `<div class="ok">✓ Tiến độ thời gian: ${fmtPct(m.timeProgress,1)} • Hoàn thành hệ thống dùng để so: ${fmtPct(m.systemPct)}</div>`
    );


    lines.push(
      `<div class="ok">✓ GAP KPI: ${fmt1(m.systemGap)} • cần ${fmt1(m.requiredPerDay)}/ngày • forecast ${fmtPct(m.forecastPct,1)}</div>`
    );
    ...
    els.previewMeta.textContent =
      `${m.rows.length} cửa hàng • ${errs.length} lỗi • ${warns.length} cảnh báo`;
  }
  ...
  function showValidationPopup(
    ...
    els.modalTitle.textContent =
      hasError
        ? 'Dữ liệu có lỗi cần xử lý'
        : 'Có dữ liệu cần kiểm tra lại';
  ...
  function showFatalPopup(message){
    ...
    els.modalTitle.textContent =
      'Không thể phân tích bảng dữ liệu';


    els.modalSummary.textContent =
      'Tool chưa tạo Dashboard mới. Vui lòng sửa dữ liệu và bấm “Phân tích dữ liệu” lại.';


    els.modalIssues.innerHTML =
      `<div class="modal-issue err">✕ ${escapeHtml(message)}</div>`;
  ...
  function drawDashboard(ctx,m){
    ...
    text(
      ctx,
      'THEO DÕI % HOÀN THÀNH THEO CỬA HÀNG',
      26,
      66,
      21,
      '#f1f4f6',
    ...
    fitText(
      ctx,
      `SỐ LIỆU CHỐT HẾT NGÀY ${dateVN(m.closingDate)}`,
      updX+updW/2,
      updY+66,
      278,
      13.2,
    ...
    fitText(
      ctx,
      `${m.currentDay}/${m.totalDays} NGÀY ĐÃ QUA`,
      327,
      y+96,
      268,
      14.8,
    ...
    const groupDefs = [
      [
        ...
        'VƯỢT NHIỀU (> +10 ĐIỂM %)',
        C.green,
        'Duy trì nhịp bán và tối ưu tồn kho.'
      ],
      ...
      [
        ...
        'KỊP (0 → +10 ĐIỂM %)',
        C.blue,
        'Bám sát kế hoạch; giữ tốc độ hiện tại.'
      ],
      ...
      [
        ...
        'CHẬM (−10 → < 0 ĐIỂM %)',
        C.orange,
        'Cần tăng tốc để về đúng tiến độ.'
      ],
      ...
      [
        ...
        'CHẬM NHIỀU (< −10 ĐIỂM %)',
        C.red,
        'Ưu tiên hành động quyết liệt ngay.'
      ]
    ];
    ...
    const scheduleValue =
      m.scheduleValueDelta;


    const scheduleValueColor =
      m.systemStatus.color;
    ...
    const aux = [
      {
        label:'ĐẠT TIẾN ĐỘ:',
        value:
          `${fmtPct(m.onTrackPct,1)}  •  ${m.onTrackCount}/${m.sorted.length} CH`,
        color:C.greenDark
      },

      {
        label:'CHẬM TIẾN ĐỘ:',
        value:
          `${fmtPct(m.slowPct,1)}  •  ${m.slowCount}/${m.sorted.length} CH`,
        color:C.red
      },
  ...
  function clearCanvas(){
    ...
    text(
      ctx,
      'Dán dữ liệu và bấm “Đối chiếu & tạo Dashboard”',
      c.width/2,
      c.height/2-10,
      20,
      '#9fb1bf',
  ...
  async function exportPng(
    ...
    try{
      ...
      button.innerHTML =
        `<span class="export-icon">…</span><span><strong>Đang xuất ${spec.tag}</strong><small>${spec.w} × ${spec.h} px</small></span>`;
      ...
      downloadBlob(
        blob,
        `KPI-He-Thong-Ban-Le-${isoDate(model.closingDate)}-${spec.tag}.png`
      );
