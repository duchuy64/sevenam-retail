...
\tTổng\t7.010\t2.839,1\t2.713,5\t125,6\t219,5\t41%`;
  ...
  function storeDisplay(code){return cleanCell(code);}
  ...
  function drawDashboard(ctx,m){
    ...
    // 2. Target completion — keep percentage only, with 1 decimal place.
    ...
    fitText(ctx,fmtPct(m.systemCalcPct,1),542,y+53,298,58,C.greenDark,'700','center',45);

    // 3. Average daily sales + month-end forecast.
    // The old monthly Target KPI card is intentionally removed.
    // This combined card expands left to use the freed space.
    const rightKpiX=717,rightKpiW=796;
    shadowCard(ctx,rightKpiX,y,rightKpiW,h,10);
    const halfW=rightKpiW/2,leftCX=rightKpiX+halfW/2,rightCX=rightKpiX+halfW+halfW/2;

    drawAssetIcon(ctx,'average_day',rightKpiX+36,y+49,60);
    text(ctx,'DOANH SỐ BÌNH QUÂN',rightKpiX+78,y+14,15.3,C.text,'700');
    text(ctx,'/ NGÀY',rightKpiX+78,y+34,15.3,C.text,'700');
    // Chỉ hiển thị doanh số bình quân/ngày + đơn vị tr; bỏ diễn giải doanh số / số ngày.
    fitText(ctx,`${fmt1(m.averagePerDay)} tr`,leftCX+12,y+55,330,57,C.greenDark,'700','center',46);

    const rightDivider=rightKpiX+halfW;
    line(ctx,rightDivider,y+13,rightDivider,y+120,'#aeb6bc',1.2);
    drawAssetIcon(ctx,'forecast',rightDivider+36,y+49,60);
    text(ctx,'DỰ ĐOÁN DOANH SỐ',rightDivider+78,y+14,15.3,C.text,'700');
    text(ctx,'CUỐI THÁNG',rightDivider+78,y+34,15.3,C.text,'700');
    // Forecast shows percentage only; no absolute revenue number.
    fitText(ctx,fmtPct(m.forecastPct,1),rightCX+12,y+53,310,58,'#f06f12','700','center',45);

    // LEFT TABLE — sorted descending by completion.
    // Bố cục cột TÊN / % / THANH TIẾN ĐỘ tự co giãn theo tên cửa hàng dài nhất.
    ...
    const rankX=56;
    // Nới khoảng cách mặc định XH → Tên CH để bảng thoáng hơn.
    const nameX=110;
    const deltaX=704;
    const statusDotX=772;
    const statusTextX=784;

    // Đo tên dài nhất theo đúng font đang hiển thị.
    // Tên viết tắt => cột tên vẫn gọn nhưng có khoảng thở rõ giữa XH / Tên / %.
    // Tên đầy đủ => cột tên tự nới, % dịch phải, thanh tiến độ tự co lại.
    ctx.save();
    ctx.font=`700 19.5px ${FONT_STACK}`;
    const longestStoreNamePx=Math.max(
      0,
      ...m.sorted.map(r=>ctx.measureText(String(r.name||'')).width)
    );
    ctx.restore();

    const nameColW=clamp(longestStoreNamePx+20,80,178);
    const pctX=nameX+nameColW+24;
    const xBar=pctX+40;
    const barRight=deltaX-42;
    const barW=Math.max(285,barRight-xBar);
    const barCenter=xBar+barW/2;
    const nameMaxW=Math.max(70,pctX-nameX-32);

    // Column order: STT | Cửa hàng | % | Tiến độ | +/- TG | Trạng thái.
    text(ctx,'STT',rankX,257,15.5,'#fff','700','center');
    text(ctx,'CỬA HÀNG',nameX,257,15.5,'#fff','700');
    text(ctx,'%',pctX,257,16,'#fff','700','center');
    text(ctx,'TIẾN ĐỘ',barCenter,249,14.5,'#fff','700','center');
    text(ctx,'THỰC ĐẠT / TARGET THÁNG',barCenter,269,11.5,'#dbe9f7','700','center');
    text(ctx,'+/- TG',deltaX,257,14.5,'#fff','700','center');
    text(ctx,'TRẠNG THÁI',824,257,14.5,'#fff','700','center');

    const rowH=35.6, startY=289;
    m.sorted.forEach((r,i)=>{
      ...
      fitTextMiddle(ctx,r.status.label,statusTextX,ry+18.5,104,13.8,r.status.color,'700','left',12.2);
    });
    ...
    // Top 5 cũng co/giãn theo tên dài nhất để tên CH không cách xa thanh tiến độ.
    const topNameX=978;
    const topPctX=1445;
    ctx.save();
    ctx.font=`700 16.5px ${FONT_STACK}`;
    const topLongestNamePx=Math.max(
      0,
      ...m.top5.map(r=>ctx.measureText(String(r.name||'')).width)
    );
    ctx.restore();
    const topBarX=clamp(topNameX+topLongestNamePx+22,1040,1140);
    const topBarRight=1410;
    const topBarW=Math.max(270,topBarRight-topBarX);
    const topNameMaxW=Math.max(58,topBarX-topNameX-14);

    m.top5.forEach((r,i)=>{
      const yy=289+i*31.0;
      circle(ctx,948,yy+14,12,r.status.color);
    ...
    // Chỉ hiển thị số chênh lệch + đơn vị tr; bỏ dòng Kế hoạch theo TG.
    fitText(ctx,`${fmtSignedWhole(scheduleValue)} tr`,1072,lowY+43,220,33,scheduleValueColor,'700','center',26);
